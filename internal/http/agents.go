...
func (h *AgentsHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /v1/teams/{id}/export/preview", h.authMiddleware(h.handleTeamExportPreview))
	mux.HandleFunc("GET /v1/teams/{id}/export", h.authMiddleware(h.handleTeamExport))
	mux.HandleFunc("POST /v1/teams/import", h.authMiddleware(h.handleTeamImport))
	// Read-only (viewer+)
